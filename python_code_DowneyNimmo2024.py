...
Uses Python
Email bgdowney@ucsc.edu or brynna.downey@swri.org
...
3. solve_KoC()
4. calculate_heat_damping()
5. graph_dissipation_partition()
6. graph_heat_damping()
7. graph_Titan_free_ocean_nutation()
8. graph_Jovian_offset
9. subplots_Jovian_offset()
...
def fcn_thin_top(M, R, R_cmb, R_icb, C, C_ice, C_int, c, J2, C22, f_cmb, f_icb, e, n):
	...
	G210 = (1-e**2)**(-3/2) # [ ] Eccentricity term, Eq. 16a
	G201 = 7/2*e - 123/16*e**3 + 489/128*e**5 # [ ] Eccentricity term, Eq. 16b
	f = J2/c # [ ] J2 term, Eq 4
	gamma = 4 * C22/c # [ ] C22 term, Eq 4
	phi_m = 3/2 * (G210*f + 1/2*G201*gamma) # [ ] Gravitational torques on whole body, Eq. 15a
	...
	one = - n * (C/C_rest) * (e_f + phi_m) 
	two = n * e_f * phi_m / (e_f + phi_m)
	return( one + two )
...
def graph_Titan_free_ocean_nutation(bounds):
	...
	# Print one case
	print('EXAMPLE')
	FON_period_test = suite(d=20e3, h=400e3, f_cmb=f_cmb_low, f_icb=f_icb0, prin=1)
	...
	''' GRAPH '''
	plt.figure(figsize = 4/5*np.array([6.4, 4.8]))
	plt.grid()
...
#graph_Titan_free_ocean_nutation(bounds=1)

''' Table 1: rotation states of Moon/Titan/Io/Ganymede '''
#end_k2oQ_M, end_KoC_M = calculate_dissipation_from_spin(Moon)
#graph_Jovian_offset(moon=Io, graph=1, k2oQ_test=0.015, precision=0, legend=1)
#graph_Jovian_offset(moon=Europa, graph=1, k2oQ_test=0.02, precision=0.05/60, legend=1)
#graph_Jovian_offset(moon=Ganymede, graph=1, k2oQ_test=0.0016, precision=1e-6*rd, legend=1)
